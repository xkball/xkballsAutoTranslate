...
//    public static String LLM_MODEL_CONFIGURATION ublic
    ...
    public static final ModConfigSpec.ConfigValue<String> HTTP_PROXY_HOST_CONFIG = BUILDER.comment("The http proxy host if not empty.Default: \"\"").define("http_proxy_host", "");
    ...
    public static final ModConfigSpec.ConfigValue<TranslatorType> TRANSLATOR_TYPE_CONFIG = BUILDER.comment("The translator. Default uses https://xkball.com/api/v1/translate.").defineEnum("translator_type",TranslatorType.DEFAULT,TranslatorType.values());
...
//    private static final ModConfigSpec.ConfigValue<String> LLM_POST_CONTENT_CONFIG = BUILDER.comment("The Json post to LLM API.DON'T modify it if you don't know what it is.").define("llm_post_content", LLMTranslate.CONTENT_TEMPLE);
    public static final ModConfigSpec.ConfigValue<String> LLM_SYSTEM_PROMPT_CONFIG = BUILDER.comment("The system prompt give to llm.Should contain ${targetLanguage} to replace to target language.Default: " + DEFAULT_SYSTEM_PROMPT).define("llm_system_prompt", DEFAULT_SYSTEM_PROMPT);
